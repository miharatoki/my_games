...
RSpec.describe 'Notificationモデルのテスト' do
  ...
  let!(:favorite_notification) do
    create(:notification, favorite_id: favorite.id,
                          sender_id: sender.id, receiver_id: receiver.id, action: 'favorite')
  end
  let!(:comment_notification) do
    create(:notification, post_comment_id: post_comment.id,
                          sender_id: sender.id, receiver_id: receiver.id, action: 'post_comment')
  end
  ...
  context '通知内容がアクションと同じか' do
    ...
    it 'コメントした場合' do
      expect(comment_notification.action).to eq 'post_comment'
    end
  end
  ...
  context '親モデルのレコードが削除された時に紐づけられたnotificationのレコードも削除されるか' do
    ...
    it 'PostCommentモデルのレコードを削除した時' do
      ...
      expect(Notification.all.count).to eq 1
      expect(Notification.
        exists?(post_comment_id: comment_notification.post_comment_id)).to eq false
    end
  end
